...
import {
  getSchoolStaffListApi,
  getSchoolStaffDetailApi,
  createSchoolStaffApi,
  updateSchoolStaffApi,
  toggleSchoolStaffStatusApi,
  deleteSchoolStaffApi,
  getSchoolOrgUnitOptionsApi,
  getSchoolRoleOptionsApi,
  staffRoleTypeMap,
} from '@/api/schoolUsers'

defineOptions({ name: 'SchoolStaffListView' })
...
const roleTypeOptions = Object.entries(staffRoleTypeMap).map(([value, label]) => ({ value, label }))
const genderOptions = [
  { value: 'male', label: '男' },
...
async function loadData() {
  ...
  try {
    ...
    const data = await getSchoolStaffListApi(tenantId.value, filters)
...
async function loadOptions() {
  try {
    const [orgs, roles] = await Promise.all([
      getSchoolOrgUnitOptionsApi(tenantId.value),
      getSchoolRoleOptionsApi(tenantId.value),
    ])
    orgUnitOptions.value = orgs
...
async function openEditDrawer(row) {
  ...
  try {
    const detail = await getSchoolStaffDetailApi(tenantId.value, row.id)
    if (detail) {
      drawerForm.value = {
        real_name: detail.real_name || '',
        role_type: detail.role_type || 'staff',
        no_work: detail.no_work || '',
        gender: detail.gender || '',
        phone: detail.account?.phone || '',
        email: detail.account?.email || '',
        office_phone: detail.office_phone || '',
        org_unit_id: detail.org_unit_id || null,
        intro: detail.intro || '',
...
async function handleDrawerSubmit() {
  ...
  try {
    if (drawerMode.value === 'create') {
      await createSchoolStaffApi(tenantId.value, drawerForm.value)
      ElMessage.success('新增教职工成功')
    } else {
      await updateSchoolStaffApi(tenantId.value, drawerForm.value._id, drawerForm.value)
      ElMessage.success('编辑教职工成功')
    }
...
async function handleToggleStatus(row) {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要${actionText}教职工「${row.real_name}」的账号吗？`,
      '确认操作',
      { type: 'warning' }
    )
    await toggleSchoolStaffStatusApi(tenantId.value, row.id, newStatus)
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm(
      `确定要删除教职工「${row.real_name}」吗？删除后不可恢复。`,
      '确认删除',
      { type: 'warning', confirmButtonText: '删除', cancelButtonText: '取消' }
    )
    await deleteSchoolStaffApi(tenantId.value, row.id)
...
async function openDetail(row) {
  ...
  try {
    detailData.value = await getSchoolStaffDetailApi(tenantId.value, row.id)
  } catch {
    ElMessage.error('加载详情失败')
  } finally {
...
function getOrgName(orgId) {
  if (!orgId) return '-'
  const org = orgUnitOptions.value.find(o => o.value === orgId)
  return org?.label || '-'
}
...
<template>
  <div class="page-container">
    <!-- 页面头部 -->
    <PageHeader title="教职工管理" description="管理学校教职工信息与账号">
      <template #actions>
        <el-button type="primary" :icon="Plus" @click="openCreateDrawer">新增教职工</el-button>
      </template>
    </PageHeader>
    ...
    <el-card shadow="hover" class="section-card table-card">
      <el-table :data="pagedData" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column label="所属组织" min-width="150">
          <template #default="{ row }">{{ getOrgName(row.org_unit_id) }}</template>
        </el-table-column>
      ...
      <!-- 空状态 -->
      <EmptyPlaceholder v-if="!loading && staffList.length === 0" text="暂无教职工数据" description="点击「新增教职工」添加第一位教职工" />
    </el-card>

    <!-- 新增/编辑抽屉 -->
    <el-drawer
      v-model="drawerVisible"
      :title="drawerMode === 'create' ? '新增教职工' : '编辑教职工'"
      size="640px"
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="12" v-if="drawerMode === 'create'">
            <el-form-item label="工号">
              <el-input v-model="drawerForm.no_work" placeholder="请输入工号" />
            </el-form-item>
          </el-col>
          <el-col :span="12" v-if="drawerMode === 'edit'">
            <el-form-item label="工号">
              <el-input v-model="drawerForm.no_work" placeholder="请输入工号" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="角色类型" prop="role_type">
              <el-select v-model="drawerForm.role_type" placeholder="请选择角色" style="width: 100%">
                <el-option
          ...
          <el-col :span="12">
            <el-form-item label="所属组织">
              <el-select v-model="drawerForm.org_unit_id" placeholder="请选择组织" clearable style="width: 100%">
                <el-option
                  v-for="opt in orgUnitOptions"
                  :key="opt.value"
                  :label="opt.label"
                  :value="opt.value"
                />
          ...
          <el-col :span="12">
            <el-form-item label="办公电话">
              <el-input v-model="drawerForm.office_phone" placeholder="请输入办公电话" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="简介">
          <el-input v-model="drawerForm.intro" type="textarea" :rows="3" placeholder="请输入简介" />
        </el-form-item>
        ...
        <template v-if="detailData">
          <div class="detail-header">
            ...
            <div class="detail-header-info">
              ...
              <span class="detail-header-sub">{{ detailData.role_label || '' }}</span>
            </div>
          </div>
          <div class="detail-section">
            ...
            <el-descriptions :column="1" border>
              ...
              <el-descriptions-item label="工号">{{ detailData.no_work || '-' }}</el-descriptions-item>
              <el-descriptions-item label="角色">{{ detailData.role_label || '-' }}</el-descriptions-item>
              <el-descriptions-item label="所属组织">{{ getOrgName(detailData.org_unit_id) }}</el-descriptions-item>
              <el-descriptions-item label="性别">{{ detailData.gender_label || '-' }}</el-descriptions-item>
              <el-descriptions-item label="办公电话">{{ detailData.office_phone || '-' }}</el-descriptions-item>
